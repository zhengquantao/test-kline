<template>
  <div id="app" class='home'>
    <div>
       <Vue-kline :klineParams="klineParams" :klineData="klineData" @refreshKlineData="refreshKlineData"  ref="callMethods"></Vue-kline>
      <div id="sidebarTheme">
      <div class="sidebar">
        <ul>
...
<script>
    ...
    export default {
        ...
        methods:{
            requestDatas(url) {
                this.$axios.get("http://127.0.0.1:5000/test?name="+url+".json").then(ret => {
                    this.klineData = ret.data;
                    // if(!this.klineData.hasOwnProperty('data')){
                    //     this.klineData = ret.data;
                    // }else{
                    this.$refs.callMethods.kline.chartMgr.getChart().updateDataAndDisplay(ret.data.data.lines);
                    // }
            ...
            refreshKlineData(option) {
                console.log(option)
                if(option > 3600000) { // 周, 日 (两年)
                    console.log("天-周 197");
                    const url = "ag1912";
                    this.requestDatas(url);
                }else if(option > 900000 ) { // 1小时, 30分钟
