...
export class ChartManager {
  ...
  drawArea(context, area, plotterNames) {
    ...
    for (i = 0; i < cnt; i++) {
      let name = areaName + plotterNames[i];
      let plotterName = this.drawIndic(name);
      plotter = this._plotters[name];
      // plotter = this._plotters[plotterName];
  ...
  drawIndic(name) {
    let str1 = "frame0.k0.indic0Range";
    let str2 = "frame0.k0.indic0";
    if (name.includes(str1) || name.includes(str2)) {
      this._indicMap[name]="test"
      return name;
    }
  }
